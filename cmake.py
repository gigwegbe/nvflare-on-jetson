...
class CMake:
    ...
    def _get_cmake_command() -> str:
        ...
        if cmake3 is not None and CMake._get_version(cmake3) >= version.parse("3.10.0"):
            cmake_command = 'cmake3'
            return cmake_command
        # elif cmake is not None and CMake._get_version(cmake) >= distutils.version.LooseVersion("3.10.0"):
        elif cmake is not None and CMake._get_version(cmake) >= version.parse("3.10.0"):
            return cmake_command
        else:
    ...
    def _get_version(cmd: str) -> Any:
        ...
        for line in check_output([cmd, '--version']).decode('utf-8').split('\n'):
            if 'version' in line:
                ...
                return version.parse(line.strip().split(' ')[2])
